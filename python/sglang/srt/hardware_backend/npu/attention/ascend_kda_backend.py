...
def _scatter_kda_conv_snapshot(
    dst: torch.Tensor,
    src: torch.Tensor,
    ...
    The installed generic scatter splits Kimi-K3's 6,912-element snapshot
    into seven 1,024-element logical tiles per layer, then serializes those
    tiles over a bounded 48-program grid. A single 8,192-element BF16 tile
    fits comfortably in A3 UB and needs only one program per request/layer.
    Return ``False`` for layouts that need the generic stride-aware fallback.
    """
    if (
        dst.ndim != 4
        or src.ndim != 5
...
def _move_kda_temporal_snapshot(
    dst: torch.Tensor,
    src: torch.Tensor,
    dst_indices: torch.Tensor,
    src_indices: torch.Tensor,
    ...
    that keeps each A3 UB tile within budget without launching thousands of
    tiny programs. Unsupported layouts keep the generic stride-aware path.
    """
    if (
        dst.ndim != 5
        or src.ndim != 6
    ...
    def __new__(cls, *args, **kwargs):
        ...
        class _AscendKDAHybrid(_Base):
            def update_mamba_state_after_mtp_verify(
                ...
                if not _move_kda_temporal_snapshot(
                    ssm_states,
                ...
                if has_conv_snapshots:
                    ...
                    if not _scatter_kda_conv_snapshot(
                        conv_states,
                        intermediate_conv_window_cache,
                        dst_indices_tensor,
                        src_indices_tensor,
